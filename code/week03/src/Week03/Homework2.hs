...
mkValidator pubKH s _ ctx =
  traceIfFalse "this is not the beneficiary address" checkSig
    && traceIfFalse "the deadline has not been reached" checkDeadline
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    checkSig :: Bool
    checkSig = pubKH `elem` txInfoSignatories info
